...
if (require.main === module) {
  ...
  switch (command) {
    ...
    case 'new':
      ...
      const title = process.argv.slice(4).join(' ') || 'New Post';
      const slug = title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
      const today = new Date().toISOString().split('T')[0];
      
      if (!['stream', 'lab', 'essays'].includes(type)) {
        console.log('Content type must be: stream, lab, or essays');
        break;
      }
...
`;
      ...
      const contentDir = `content/${type}`;
      if (!fs.existsSync(contentDir)) {
        fs.mkdirSync(contentDir, { recursive: true });
      }
      
      fs.writeFileSync(`${contentDir}/${slug}.md`, newPost);
      console.log(`Created new ${type}: ${contentDir}/${slug}.md`);
      break;
...
module.exports = EnhancedBlogBuilder;
